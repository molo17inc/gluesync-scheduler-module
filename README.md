...
The polling uses CoreHub's `GET /pipelines/{pipeline_id}/entities-status` endpoint, which returns the same runtime status the MPP UI uses to render Active / Hold / Error / Warning. An entity is considered settled (ready to proceed) when it reports Hold, Error, or Warning — including an explicit `status`/`warningState`/`errorState` — even if Hub still reports sync/migration flags. This mirrors the CoreHub / MPP UI computation, where Warning is a distinct settled state alongside Hold and Error.
