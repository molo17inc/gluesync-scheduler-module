...
def add_routing_column(engine):
    ...
    if 'routing' in existing_columns:
        logger.info("MIGRATION: routing column already exists, skipping migration")
        return

    logger.info("MIGRATION: Adding routing column")

    try:
        with engine.connect() as connection:
            add_column_ddl = DDL("ALTER TABLE trigger_flows ADD COLUMN routing VARCHAR")
            connection.execute(add_column_ddl)
            connection.commit()
            logger.info("MIGRATION: routing column added")
...
def main():
    parser = argparse.ArgumentParser(description="Add routing column to trigger_flows table")
